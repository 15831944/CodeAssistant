...
HRESULT COleImage::OnDraw(ATL_DRAWINFO& di)
{
	...
	// 得到图片数据
	CBitmap * m_pMap = m_pDCs[m_pFrmIndexes[m_iCurTime]].GetCurrentBitmap();
	BITMAP bm;    
	m_pMap->GetBitmap(&bm);
	...
	if (m_nFrmCount > 0)
	{
		...
		SetStretchBltMode(di.hdcDraw, HALFTONE);
		StretchBlt(di.hdcDraw, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, m_pDCs[m_pFrmIndexes[m_iCurTime]], 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
	}
	return S_OK;
}
